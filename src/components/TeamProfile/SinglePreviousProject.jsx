...
import EditPreviousProjectModal from './Modals/EditPreviousProjectModal'

const SinglePreviousProject = ({ value }) => {
    const [seeMore, setSeeMore] = useState(false)
    const [editPreviousProjectModal, setEditPreviousProjectModal] = useState(false)

    return (
        <>
            <div className='flex gap-[10px] justify-between items-start'>
                <img src={user} className='w-[56px] rounded-full min-w-[30px]' />
                <div className='flex-1 flex flex-col gap-[0.390625vw] pt-[8px]'>
                    <h1 className='text-custom-16 font-[700] leading-[16px]'>{value.name}</h1>
            ...
            {editPreviousProjectModal && <EditPreviousProjectModal setModal={setEditPreviousProjectModal} />}
        </>
    )
}

export default SinglePreviousProject